...
async def test(ctx):
    ...
    msg = await ctx.send(f">>> A sessão vai começar mais cedo provavelmente as 3:00 / 3:30 só estar todo mundo presente que começamos então quem tiver livre continue\nTempo esperado para começar:{i}")
    while(i != 0):
        await msg.edit(content=f">>> A sessão vai começar mais cedo provavelmente as 3:00 / 3:30 só estar todo mundo presente que começamos então quem tiver livre continue\nTempo esperado para começar:{math.floor(i/60)}m")
